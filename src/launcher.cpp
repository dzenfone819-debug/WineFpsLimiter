...
// Helper: inject a DLL into an already-running process by PID
static bool InjectIntoProcessByPID(DWORD pid, const std::string &dllPath) {
    ...
    void* pRemotePath = VirtualAllocEx(hProc, NULL, dllPath.size() + 1, MEM_COMMIT, PAGE_READWRITE);
    if (!pRemotePath) { CloseHandle(hProc); return false; }
    SIZE_T written = 0;
    if (!WriteProcessMemory(hProc, pRemotePath, dllPath.c_str(), dllPath.size() + 1, &written)) {
...
void Inject(const GameProfile& game, HWND hwndLauncher) {
    ...
    // Choose appropriate injector DLL matching the game's architecture.
    std::string dllName = "FPSLimiter_x64.dll";
    if (binType == SCS_32BIT_BINARY) dllName = "FPSLimiter_x86.dll";

    std::string fullDllPath = launcherDir + dllName;

    // Warn if the launcher architecture mismatches the game (optional)
// Replace blocking MessageBox on arch mismatch with a non-blocking log to `launcher.log`.
auto LogLauncher = [&](const char* msg){
    char path[MAX_PATH];
    GetModuleFileNameA(NULL, path, MAX_PATH);
    std::string p = path;
    size_t s = p.find_last_of("\\/");
    if (s != std::string::npos) p = p.substr(0, s+1);
    p += "launcher.log";
...
#ifdef _WIN64
    if (binType == SCS_32BIT_BINARY) LogLauncher("Warning: x64 Launcher -> x86 Game mismatch.");
#else
    if (binType == SCS_64BIT_BINARY) LogLauncher("Warning: x86 Launcher -> x64 Game mismatch.");
#endif
    ...
    std::string cmd = std::string("\"") + game.Path + "\"";
    std::vector<char> cmdBuf(cmd.begin(), cmd.end());
    cmdBuf.push_back('\0');

    // Use lpApplicationName to avoid Windows parsing issues and ensure correct executable is used.
    if (CreateProcessA(game.Path, cmdBuf.data(), NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, gameDir.c_str(), &si, &pi)) {
#ifdef _WIN64
        if (binType == SCS_32BIT_BINARY) {
            bool launchedHelper = false;
#ifdef build_x86_FPSInjector_exe_len
            // Write embedded x86 helper to temp and launch it
            char tmpPath[MAX_PATH];
            DWORD tp = GetTempPathA(MAX_PATH, tmpPath);
            if (tp && tp < MAX_PATH) {
                char outPath[MAX_PATH];
                unsigned long long uniq = GetTickCount64();
                snprintf(outPath, MAX_PATH, "%sfps_injector_%llu.exe", tmpPath, uniq);
                HANDLE hf = CreateFileA(outPath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                if (hf != INVALID_HANDLE_VALUE) {
                    DWORD written = 0;
                    WriteFile(hf, build_x86_FPSInjector_exe, (DWORD)build_x86_FPSInjector_exe_len, &written, NULL);
                    CloseHandle(hf);
                    std::string cmdline = std::string("\"") + outPath + "\" \"" + game.Path + "\" \"" + fullDllPath + "\"";
                    STARTUPINFOA hsi = {0}; hsi.cb = sizeof(hsi);
                    PROCESS_INFORMATION hpi = {0};
                    if (CreateProcessA(NULL, &cmdline[0], NULL, NULL, FALSE, 0, NULL, NULL, &hsi, &hpi)) {
                        // Launch background deleter thread
                        struct DelCtx { HANDLE hProc; char* path; };
                        DelCtx* ctx = (DelCtx*)malloc(sizeof(DelCtx));
                        ctx->hProc = hpi.hProcess;
                        ctx->path = _strdup(outPath);
                        CloseHandle(hpi.hThread);
                        DWORD (__stdcall *threadFunc)(void*) = [](void* a)->DWORD {
                            DelCtx* c = (DelCtx*)a;
                            WaitForSingleObject(c->hProc, INFINITE);
                            CloseHandle(c->hProc);
                            DeleteFileA(c->path);
                            free(c->path);
                            free(c);
                        ...
                        HANDLE hDel = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)threadFunc, ctx, 0, NULL);
                        if (hDel) CloseHandle(hDel);
...
#endif
            if (!launchedHelper) {
                // Fallback: external helper next to launcher
                std::string helperPath = launcherDir + "FPSInjector.exe";
                std::string cmdline = std::string("\"") + helperPath + "\" \"" + game.Path + "\" \"" + fullDllPath + "\"";
                STARTUPINFOA hsi = {0}; hsi.cb = sizeof(hsi);
                PROCESS_INFORMATION hpi = {0};
                if (!CreateProcessA(NULL, &cmdline[0], NULL, NULL, FALSE, 0, NULL, NULL, &hsi, &hpi)) {
                    LogLauncher("Failed to launch x86 helper for injection");
                ...
                // Embedded helper launched successfully; close launcher-created handles and return
                CloseHandle(pi.hProcess);
                CloseHandle(pi.hThread);
                return;
            }
        ...
        // Same-architecture injection (or non-WOW case): perform injection here.
        void* pLoadLibrary = (void*)GetProcAddress(GetModuleHandleA("kernel32.dll"), "LoadLibraryA");
        ...
        if (hThread) {
            ...
            // Suspended injection failed — attempt fallback: terminate suspended process and relaunch without suspension,
            // wait briefly, then inject into running process.
            DWORD err = GetLastError();
            char buf[256];
            snprintf(buf, sizeof(buf), "Suspended injection failed. Error=%lu. Attempting delayed injection fallback.", (unsigned long)err);
            LogLauncher(buf);
            ...
            // Launch again without suspension
            STARTUPINFOA si2;
            ...
            if (CreateProcessA(game.Path, cmdBuf.data(), NULL, NULL, FALSE, 0, NULL, gameDir.c_str(), &si2, &pi2)) {
                // Wait a short time for the game to initialize
                Sleep(1500);
                bool injOk = InjectIntoProcessByPID(pi2.dwProcessId, fullDllPath);
                if (injOk) {
                    ShowWindow(hwndLauncher, SW_MINIMIZE);
                } else {
                    snprintf(buf, sizeof(buf), "Delayed injection failed for PID=%lu", (unsigned long)pi2.dwProcessId);
                    LogLauncher(buf);
                }
                ...
                snprintf(buf, sizeof(buf), "Failed to relaunch process for delayed injection. Error=%lu", (unsigned long)err2);
                LogLauncher(buf);
            }
        }
        }
