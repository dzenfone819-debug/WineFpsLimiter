...
auto LogLauncher = [&](const char* msg){
    ...
    if (CreateProcessA(game.Path, cmdBuf.data(), NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, gameDir.c_str(), &si, &pi)) {
        // If current launcher is 64-bit and target is 32-bit, delegate injection to x86 helper.
#ifdef _WIN64
        if (binType == SCS_32BIT_BINARY) {
            // Build helper path: FPSInjector.exe (x86 build) located next to launcher.
            // If we have the x86 helper embedded, extract to temp and run it; otherwise fall back to external helper.
#ifdef _WIN64
            bool launchedHelper = false;
#if defined(_WIN64)
            // Embedded data generated by CMake: build_x86_FPSInjector_exe[] / build_x86_FPSInjector_exe_len
            #ifdef build_x86_FPSInjector_exe_len
            {
                ...
                if (tp && tp < MAX_PATH) {
                    ...
                    // Write embedded bytes to outPath
                    ...
                    if (hf != INVALID_HANDLE_VALUE) {
                        ...
                        CloseHandle(hf);
                        // Build command line: "<outPath>" "<gamePath>" "<dllPath>"
                        std::string args = std::string("\"") + outPath + "\" \"" + game.Path + "\" \"" + fullDllPath + "\"";
                        STARTUPINFOA hsi = { sizeof(hsi) };
                        PROCESS_INFORMATION hpi = { 0 };
                        std::string cmdline = std::string("\"") + outPath + "\" " + std::string("\"") + game.Path + "\" \"" + fullDllPath + "\"";
                        if (!CreateProcessA(NULL, &cmdline[0], NULL, NULL, FALSE, 0, NULL, NULL, &hsi, &hpi)) {
                            // failed to launch embedded helper
                            DeleteFileA(outPath);
                        } else {
                            // Start a background thread to wait for helper exit and delete the temp file
...
#endif
            if (!launchedHelper) {
                // Fallback: launch external helper next to launcher (legacy behavior)
                std::string helperPath = launcherDir + "FPSInjector.exe";
                std::string cmdline = std::string("\"") + helperPath + "\" \"" + game.Path + "\" \"" + fullDllPath + "\"";
                STARTUPINFOA hsi = { sizeof(hsi) };
                PROCESS_INFORMATION hpi = { 0 };
                if (!CreateProcessA(NULL, &cmdline[0], NULL, NULL, FALSE, 0, NULL, NULL, &hsi, &hpi)) {
                    MessageBoxA(0, "Failed to launch x86 helper for injection", "Error", 0);
